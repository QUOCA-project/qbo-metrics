...
windows centred on the reference dates. \\
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\textwidth]{reference_timeseries.pdf}
...
and latitude to form the cycle anomaly $x'_c(\tau, p, \phi)$, but the cycle fields are not detrended.  Thus
detrending affects identification of onset events only.  For cycle anomaly $x'_c$, the composite is
\begin{equation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\textwidth]{lag_height_composite.pdf}
...
For zonal wind, we use the Gaussian $D_0$ by default. For temperature and
ozone, we use the sum of $D_0$ and $D_2$:
$D_2$ looks like a central Gaussian with two side-lobes, with $D_0$ included to fit the central amplitude.
This allows a better fit with both the equatorial and subtropical contributions. Ozone is represented internally
as \texttt{o3}; CF-compliant input is identified from the standard name
\texttt{mole\_fraction\_of\_ozone\_in\_air}.

The latitudinal width is calcualted as the full width at half maximum (FWHM) of the absolute fitted equatorial
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{latitudinal_width_fits.pdf}
...
at the equator is then composited and fitted as a function of lag in days.  As for monthly cycle fields, these
daily anomalies are not detrended, and the final reported period is an average of the calculation for a
westerly- and easterly-onset composite. Figure~\ref{fig:daily-period} shows the fit calculation on
daily-sampled composites.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{daily_period_fits.pdf}
\caption{Daily ERA5 30-hPa composites and sine fits.  The periods for different choices of onset
differ by 60.8 days and average to 850.9 days (27.96 months).}
...
Figure~\ref{fig:phase-amplitude} shows the phase and amplitude profiles for zonal wind and temperature using
the daily-sampled QBO period of 27.96 months with the vertical extent and the reference height (30 hPa)
indicated.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.94\textwidth]{phase_amplitude.pdf}
\caption{Amplitude and phase profiles for a QBO period of 27.96 months from ERA5.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\textwidth]{descent_rate.pdf}
...
\end{equation}
Figure~\ref{fig:coherence} shows the QBO wind anomalies and cycle/composite
coherence.

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{cycle_coherence.pdf}
\caption{QBO-cycle anomalies and pressure-resolved coherence on a continuous
timeline. Vertical dashed lines mark westerly onsets; the
...
estimate and monthly data otherwise.  Of 19 westerly and 18 easterly onsets,
17 and 18, respectively, have complete cycle windows.  Table
\ref{tab:example-results} illustrates the resulting metrics; the values are
not prescribed observational targets.

\begin{table}[htbp]
\centering
\caption{Results from ERA5 data.}
...
Mean descent rate, easterly-/westerly-onset composite & 4.78 / 7.41 $\mathrm{hPa\,month^{-1}}$ \\
Mean westerly-onset cycle coherence & 0.57 \\
Westerly-onset wind FWHM near 50 hPa & $22.5^{\circ}$ \\
Westerly-onset temperature FWHM near 30 hPa & $16.2^{\circ}$ \\
\bottomrule
\end{tabularx}
\end{table}

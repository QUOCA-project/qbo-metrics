...
This document defines composite and EOF methods for characterising the
...
The EOF method estimates period, amplitude and variance explained from the
monthly equatorial zonal wind.

The composite methodology is a variation of the QBOi methodology, using 30 hPa as the reference pressure level
and extending the method to include latitudinal dependence. See \citet{pahlavan2021, simpson2025, yu2017,
lee2024} for other examples of a QBO composite methodology. The two-EOF phase-space view follows
\citet{wallace1993}, using period and amplitude definitions and code from \citet{dallasanta2021}.

See \texttt{example.py} for a demonstration of both the EOF and composite calculations.
...
EOF metrics & Period, mean amplitude and variance explained from the first
two log-pressure-weighted wind EOFs between 10 and 100~hPa. \\
...
\section{Input data and workflow}

The code accepts daily or monthly input data. Monthly means are the recommended input for composites. The loader
resamples daily data to monthly means before compositing. If provided, daily data can be used to obtain a more
accurate period estimate. All other composite diagnostics use monthly data. If a longitude axis is provided in
the input data, it is reduced to a zonal mean.

Set \texttt{DATA\_LOC} to a path containing both zonal wind and temperature, or provide a file list for separate
wind and temperature files:
\begin{verbatim}
DATA_LOC = ["/data/zonal_wind.nc", "/data/temperature.nc"]
\end{verbatim}

An example minimal data file \texttt{example\_data.nc} is included in the GitHub repository,
containing both \texttt{u} and \texttt{T}. Separate files need compatible time and spatial coordinates. The
loader recognises aliases for zonal wind, temperature, and trace gases including O3, NOx, NOy, HNO3, HCl, N2O,
CH4, CFC12, and CO.

\section{EOF methodology}

The EOF method uses monthly zonal-mean zonal wind averaged over the chosen
equatorial latitude band, $5^{\circ}$S--$5^{\circ}$N by default. The input has
pressure and time dimensions, in that order. Pressure is in hPa, and wind is
in m/s. The default analysis range is 10--100~hPa. No onset dates or cycle
exclusions enter this method.

At each pressure, subtract the time-mean wind. Weight the anomaly by the square
root of its normalised log-pressure spacing, then calculate its singular value
decomposition. Scale each of the first two physical EOFs to a peak of $+1$.
Apply the inverse scale to their principal components, which retain wind units.
The mean length of the two-component principal-component vector defines amplitude. The sum of their eigenvalues
divided by the total eigenvalue sum defines variance explained. The phase speed of the standardised components
gives the period; the default uses its mean, while \texttt{period\_mode="median"} uses its median.

Figure~\ref{fig:eof} shows the EOF patterns, standardised principal components, phase and phase speed following
figure A1 of \citet{dallasanta2021}. Figure~\ref{fig:eof-reconstruction} compares the input wind with its two-EOF
reconstruction.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{eof_diagnostics.pdf}
\caption{ERA5 leading EOF wind patterns, principal-component trajectory,
phase, and phase speed for the full 1979--2022 monthly record. The wind is
averaged over $5^{\circ}$S--$5^{\circ}$N.}
\label{fig:eof}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{eof_reconstruction.pdf}
\caption{ERA5 zonal wind averaged over $5^{\circ}$S--$5^{\circ}$N, its two-EOF
reconstruction, and the residual for 1979--2022. Both wind panels use the
same colour scale.}
\label{fig:eof-reconstruction}
\end{figure}
...
To calculate the QBO period with sub-monthly precision, the code can estimate the period using daily
...
\caption{$5^{\circ}$S--$5^{\circ}$N mean westerly-onset composites from ERA5. Filled contours are temperature
...
\subsection{QBO metrics}
\label{sec:metrics}

Use the parameter \texttt{COMPOSITE\_LAT\_RANGE} to control the size of the latitude mean applied to the
composite for computing metrics. It defaults to the equator, using \texttt{COMPOSITE\_LAT\_RANGE = (0.0,
0.0)}. The range $5^{\circ}$S--$5^{\circ}$N may be more appropriate for less finely resolved data. Width
diagnostics use the full latitude-resolved composite.

\subsubsection{Latitudinal width}
...
pressure levels. The continuity check applies only when both neighbouring log-pressure gaps are at most 0.15.
Sparse pressure grids cannot resolve local smoothness, so only the residual test applies there.
These thresholds are configurable. Figure~\ref{fig:width-fits}
...
\subsubsection{Period, amplitude, phase, and vertical extent}
...
If daily data is supplied, the example calculates the QBO period by applying the same compositing and
...
\subsubsection{Cycle-to-cycle coherence}
...
\bottomrule
\endlastfoot
\multicolumn{3}{@{}l}{\textit{EOF calculation}}\\[0.25em]
EOF latitude band & $5^{\circ}$S--$5^{\circ}$N & Average the monthly zonal wind. \\
EOF pressure range & 10--100 hPa & Levels used for the two-mode analysis. \\
EOF retained modes & 2 & Leading modes used for the QBO reconstruction. \\
EOF period mode & Mean & Average phase speed; median is an option. \\
\addlinespace
...
Composite latitude band & $0^{\circ}$ & Average for metrics
that do not require latitude-resolved fields. \\
\addlinespace
\multicolumn{3}{@{}l}{\textit{Period}}\\[0.25em]
Daily period use & Automatic & Use daily data when available; otherwise use
monthly composites. \\
...
\addlinespace
\multicolumn{3}{@{}l}{\textit{Composite plots}}\\[0.25em]
Colourbar limits & Automatic & Set an explicit minimum and maximum with
\texttt{colorbar\_limits}. \\
Horizontal-axis limits & Automatic & Set an explicit minimum and maximum with
\texttt{x\_limits}. \\
Pressure-axis limits & 1--200 hPa & Set the displayed pressure range with
\texttt{pres\_range}. \\
Colourbar label & None & Set text with \texttt{colorbar\_label}. \\
\end{longtable}
\normalsize
...
The figures use ERA5 for 1979--2022. Daily wind provides the period estimate.
Monthly means provide all other diagnostics. The composites exclude cycles with
onsets in 2015 and 2020, representing the 2016 and 2020 disruptions.
The onset-period diagnostics use the same exclusions.  The cycle-to-cycle
...
\ref{tab:example-results} shows the QBO metrics from the EOF and composite methodologies.

\begin{table}[htbp]
\centering
\caption{ERA5 results. Composite and onset-period metrics exclude the
disruption-onset years; EOF metrics use the full 1979--2022 record averaged
over $5^{\circ}$S--$5^{\circ}$N; variability metrics
include both onset years.}
...
Monthly-sampled QBO period (easterly-/westerly-onset) & 28.54 / 27.30 months \\
...
EOF QBO period (10--100 hPa) & 28.82 months \\
EOF QBO amplitude (10--100 hPa) & 22.92 $\mathrm{m\,s^{-1}}$ \\
EOF variance explained (first two modes) & 95.21\% \\
Individual zero-crossing period range & 18.47--37.46 months \\
Easterly--westerly period difference & 53.5 days \\
Top vertical extent, easterly/westerly composites & 0.81 / 0.77 hPa \\
Bottom vertical extent, easterly/westerly composites & 89.32 / 87.19 hPa \\
Mean descent rate, easterly-/westerly-onset composite & 4.88 / 7.18 $\mathrm{hPa\,month^{-1}}$ \\
Mean westerly-onset cycle coherence & 0.53 \\
Westerly-onset wind FWHM near 50 hPa & $22.8^{\circ}$ \\
Westerly-onset temperature FWHM near 30 hPa & $16.7^{\circ}$ \\
\bottomrule
\end{tabularx}
\end{table}

\begin{thebibliography}{9}
\bibitem[Wallace et al.(1993)]{wallace1993}
J. M. Wallace, R. L. Panetta, and J. Estberg,
``Representation of the equatorial stratospheric quasi-biennial oscillation
in EOF phase space,'' \emph{Journal of the Atmospheric Sciences}, 50,
1751--1762, 1993.
\href{https://journals.ametsoc.org/view/journals/atsc/50/12/1520-0469_1993_050_1751_rotesq_2_0_co_2.xml}
{Journal article}.

\bibitem[DallaSanta et al.(2021)]{dallasanta2021}
K. DallaSanta, C. Orbe, D. Rind, L. Nazarenko, and J. Jonas,
``Dynamical and trace gas responses of the quasi-biennial oscillation to
increased CO$_2$,'' \emph{Journal of Geophysical Research: Atmospheres},
126, e2020JD034151, 2021.
\href{https://doi.org/10.1029/2020JD034151}
